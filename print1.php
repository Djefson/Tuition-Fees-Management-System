...
$header=array('Date','Semester','Amount','Description','Bank Name','Account','Slip Number');
...
$pdf->Cell(2);
$pdf->Cell(10,3.5,"Student's Tuition Fees: $kiki, $kiki1, $kiki2 on $d",0,1);

$query="SELECT `student`.`stdid`, `student`.`stdname`,`student`.`departid`, `feepayement`.`stdid`, `feepayement`.`paymentdate`,`feepayement`.`feetype`, `feepayement`.`feedescription`, `feepayement`.`semester`, `feepayement`.`academicyear`, `feepayement`.`feesamount`, `feepayement`.`bankname`, `feepayement`.`bankaccount`, `feepayement`.`slipnumber` FROM student, feepayement WHERE `feepayement`.`stdid`='$id' AND `feepayement`.`stdid`=`student`.`stdid` AND `feepayement`.`academicyear`='$y';";
...
echo '<center><h2>Click Here <a href=schedule.php><img src=images/button_1.png></a> to go back</h2></center>';
}else if(mysql_num_rows($resultat)!=0)
{
$total=0;
while($list=mysql_fetch_object($resultat))
  {
  $pdf->cell(3.5,0.7,$list->paymentdate,1,0,'C',$fond);
   $pdf->cell(2.2,0.7,$list->semester,1,0,'C',$fond);
   $pdf->cell(2.2,0.7,$list->feesamount,1,0,'C',$fond);
   $pdf->cell(3.5,0.7,$list->feedescription,1,0,'C',$fond);
   $pdf->cell(2.2,0.7,$list->bankname,1,0,'C',$fond); 
   $pdf->cell(2.2,0.7,$list->bankaccount,1,0,'C',$fond);
   $pdf->cell(2.2,0.7,$list->slipnumber,1,0,'C',$fond);
   ...
   $fond=!$fond;
    ...
    $fond=!$fond;
	$pdf->Cell(10,3.5,"The Total Amount Payed in $y is :  $total",0,1);
	$pdf->SetFont('Arial','B',10);
