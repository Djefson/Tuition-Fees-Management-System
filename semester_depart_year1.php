...
$departid=$_REQUEST['departement'];
$year=$_REQUEST['academicyear'];
$semester=$_REQUEST['semester'];
$field=$_REQUEST['field'];
$field1=strtoupper($field);
$d=date("D M d,Y");
require("fpdf.php");
...
$header=array('Student Names','Student ID','Departement',$field);
...
require"connect.php";
$pdf->SetXY(2,1);
$query1="SELECT `student`.`stdid` , `student`.`stdname` , `student`.`departid` , `feepayement`.`stdid` , `feepayement`.`paymentdate` , `feepayement`.`feetype` , `feepayement`.`feedescription` , `feepayement`.`semester` , `feepayement`.`academicyear` , `feepayement`.`bankname` , `feepayement`.`bankaccount` , `feepayement`.`slipnumber` , `paymentdetail`.`$field` AS 'field'
FROM student, feepayement, paymentdetail
WHERE `feepayement`.`stdid` = `student`.`stdid`
AND `feepayement`.`academicyear` = '$year'
AND `feepayement`.`feeid` = `paymentdetail`.`feeid`
AND `paymentdetail`.`$field`!='0'
AND `student`.`departid` = '$departid'
AND `feepayement`.`semester` = '$semester'
...
$pdf->Cell(2);
$pdf->Cell(10,3.5,"$field1 TUITION FEES REPORT: $year on $d",0,1);

$query="SELECT `student`.`stdid` , `student`.`stdname` , `student`.`departid` , `feepayement`.`stdid` , `feepayement`.`paymentdate` , `feepayement`.`feetype` , `feepayement`.`feedescription` , `feepayement`.`semester` , `feepayement`.`academicyear` , `feepayement`.`bankname` , `feepayement`.`bankaccount` , `feepayement`.`slipnumber` , `paymentdetail`.`$field` AS 'field'
FROM student, feepayement, paymentdetail
WHERE `feepayement`.`stdid` = `student`.`stdid`
AND `feepayement`.`academicyear` = '$year'
AND `feepayement`.`feeid` = `paymentdetail`.`feeid`
...
while($list=mysql_fetch_object($resultat))
  {
   ...
   $pdf->cell(2.2,0.7,$list->departid,1,0,'C',$fond);
   $pdf->cell(3.5,0.7,$list->field,1,0,'C',$fond);
