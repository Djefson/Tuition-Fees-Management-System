...
require"connect.php";
$pdf->SetXY(2,1);
$query1="SELECT `student`.`stdid` , `student`.`stdname` , `student`.`departid` , `feepayement`.`stdid` , `feepayement`.`paymentdate` , `feepayement`.`feetype` , `feepayement`.`feedescription` , `feepayement`.`semester` , `feepayement`.`academicyear` , SUM( `feepayement`.`feesamount` ) AS tot, `feepayement`.`bankname` , `feepayement`.`bankaccount` , `feepayement`.`slipnumber`, `feepayement`.`semester`
FROM student, feepayement
WHERE `student`.`departid` = '$departid'
AND `feepayement`.`stdid` = `student`.`stdid`
...
$pdf->Cell(2);
$pdf->Cell(10,3.5,"STUDENT'S TUITION FEES REPORT: $departid, $year on $d",0,1);
...
while($list=mysql_fetch_object($resultat))
  {
   ...
   $fond=!$fond;
    ...
    $fond=!$fond;
	$pdf->Cell(10,3.5,"The Total Amount Payed in $year, Departement $departid is :  $total",0,1);
	$pdf->SetFont('Arial','B',10);
