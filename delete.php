...
<!-- content -->
	...
	$a=$_GET['id'];
	include"connect.php";
	$query=mysql_query("DELETE FROM `tuition`.`student` WHERE `student`.`stdid` = '$a' LIMIT 1")or die("<h2>Can't delete, an error occur :</h2>".mysql_error());
	if(!empty($query))
	{
		echo"<h3>Record deleted successfully  !!!! </h3>";
		echo"<a href='students.php' class='button right'>Continue</a>";
	}
	else
	{
